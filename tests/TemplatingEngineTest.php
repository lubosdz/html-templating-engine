...
class TemplatingEngineTest extends TestCase
{
	...
	*/
	public function testModelsAndScalars()
	{
		...
		$ts = strtotime($client->datetime_created);
		$expectedHtml = 'Customer created on '.date($engine->defaultDateFormat, $ts).' at '.date($engine->defaultTimeFormat, $ts).'.';
		$this->assertEquals($expectedHtml, $resultHtml);

		$html = 'Generated {{ today }} at {{ now | time }}.';
		$resultHtml = $engine->render($html);
		$time = date($engine->defaultTimeFormat);
		$today = date($engine->defaultDateFormat);
